-- 1. View hotels and their managers, as well as the contact info for the managers
SELECT H.HotelName,
		CONCAT(S.FirstName, ' ', S.LastName) AS "Manager Name",
        S.PhoneNumber,
        S.Email
...
SELECT RM.RoomID
		...
		FROM ROOM RM
			...
			LEFT JOIN RESERVATION RV
				on RR.ReservationID = RV.ReservationID
			LEFT JOIN HOTEL HT
				ON HT.HotelID = RM.HotelID
        WHERE RM.HotelID = 1;
...
SELECT RM.RoomID
		...
		FROM ROOM RM
			...
			LEFT JOIN HOTEL HT
				ON HT.HotelID = RM.HotelID
        WHERE RM.HotelID = 2
			AND SYSDATE() BETWEEN RV.CheckIn AND RV.CheckOut;
...
-- 7. View current reservations and reservation information by customer --> All active reservations for customer 11
 SELECT RM.RoomID
		...
		FROM ROOM RM
			...
			LEFT JOIN HOTEL HT
				ON HT.HotelID = RM.HotelID
        WHERE RV.CustomerID = 11
				AND SYSDATE() BETWEEN RV.CheckIn AND RV.CheckOut;

-- 8. View who is working at a hotel (shifts) and what position (staff) the employee is filling --> Distinct list of staff and thier positions at hotel 1
SELECT DISTINCT
		StaffID
        ,ShiftPosition
		FROM SHIFT
...
SELECT RM.RoomID
		...
		FROM ROOM RM
			...
			LEFT JOIN HOTEL HT
				ON HT.HotelID = RM.HotelID
        WHERE HT.City = 'Charleston'
				AND SYSDATE() NOT BETWEEN RV.CheckIn AND RV.CheckOut;
